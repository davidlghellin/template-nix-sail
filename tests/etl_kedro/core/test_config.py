...
@pytest.mark.parametrize("ruta", ["C:\\datos\\ciudades.csv", "\\\\servidor\\datos\\x"])
def test_una_ruta_absoluta_de_windows_escapa_de_la_raiz(ruta):
    assert Config(entorno="pro", raiz="/lago").resolver(ruta) == ruta


@pytest.mark.parametrize("ruta", ["/datos/x.csv", "\\datos\\x.csv"])
